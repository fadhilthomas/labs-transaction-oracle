...
namespace PROJECT_UAS
{
    public partial class FRM_PENERIMAAN : Form
    {
        ...
        {
            con.Close();
            cmd = new OracleCommand("select sub_total from tbl_penerimaan where no_penerimaan = '" + txtNoPenerimaan.Text + "'", con);
        ...
        private void Otomatis()
        {
            ...
            con.Close();
            con.Open();
            cmd = new OracleCommand("select no_penerimaan from tbl_penerimaan_detail where no_penerimaan in(select max(no_penerimaan) from tbl_penerimaan_detail) order by no_penerimaan desc", con);
        ...
        private void DisplayData()
        {
            try
            {
                con.Close();
                con.Open();
                cmd = new OracleCommand("select * from tbl_penerimaan order by no_penerimaan ASC", con);
                rd = cmd.ExecuteReader();
        ...
        private void DisplayDataBar()
        {
            try
            {
                con.Close();
                con.Open();
                cmd = new OracleCommand("select kode_barang, nama_barang, harga, stock from tbl_barang where kode_barang like 'BRGA%' order by kode_barang ASC", con);
                rd = cmd.ExecuteReader();
        ...
        private void CariDataBar()
        {
            try
            {
                ...
                con.Open();
                DataTable dt = new DataTable();
                cmd = new OracleCommand("select kode_barang, nama_barang, harga, stock from tbl_barang where kode_barang like 'BRGA%' and (kode_barang LIKE '%" + txtCariBarang.Text + "%' OR nama_barang LIKE '%" + txtCariBarang.Text + "%')", con);
                rd = cmd.ExecuteReader();
                dt.Load(rd);
                dGBar.DataSource = dt;
                dGBar.Columns[0].HeaderText = "Kode Barang";
        ...
        private void btTambah_Click(object sender, EventArgs e)
        {
            ...
            {
                con.Close();
                cmd = new OracleCommand("select no_penerimaan, kode_barang from tbl_penerimaan where no_penerimaan = '" + txtNoPenerimaan.Text + "' and kode_barang = '" + txtKodeBarang.Text + "'", con);
                OracleDataReader dbr;
                con.Open();
                dbr = cmd.ExecuteReader();
                int count = 0;
                ...
                if (count == 0)
                {
                    try
                    {
                        cmd = new OracleCommand("insert into tbl_penerimaan(no_penerimaan, tgl_penerimaan, kode_barang, jumlah_terima, sub_total, keterangan) values(:no_penerimaan, :tgl_penerimaan, :kode_barang, :jumlah_terima, :sub_total, :keterangan)", con);
                        con.Close();
                        con.Open();
        ...
        private void btHapus_Click(object sender, EventArgs e)
        {
            ...
            {
                cmd = new OracleCommand("delete tbl_penerimaan where no_penerimaan=:no_penerimaan AND kode_barang=:kode_barang", con);
                con.Open();
                cmd.Parameters.Add(new OracleParameter(":no_penerimaan", txtNoPenerimaan.Text));
                cmd.Parameters.Add(new OracleParameter(":kode_barang", txtKodeBarang.Text));
                cmd.ExecuteNonQuery();
        ...
        private void btSimpan_Click(object sender, EventArgs e)
        {
            ...
            {
                cmd = new OracleCommand("insert into tbl_penerimaan_detail(no_penerimaan, tgl_penerimaan, kode_barang, jumlah_terima, sub_total, keterangan) select no_penerimaan, tgl_penerimaan, kode_barang, jumlah_terima, sub_total, keterangan from tbl_penerimaan ", con);
                con.Open();
                cmd.ExecuteNonQuery();
                con.Close();
                cmd = new OracleCommand("delete tbl_penerimaan where no_penerimaan=:no_penerimaan", con);
                cmd.Parameters.Add(new OracleParameter(":no_penerimaan", txtNoPenerimaan.Text));
                con.Open();
                cmd.ExecuteNonQuery();
                con.Close();
